...
def getMass(lvec):
    return np.sqrt(lvec[3]*lvec[3] - lvec[2]*lvec[2] - lvec[1]*lvec[1] - lvec[0]*lvec[0])
...
def Zee_savecut(result):
    return np.ones(result["Ele_Gen_E"].shape, dtype=bool)
...
class Extract:
    ...
    def readfakes(self):
        ...
        for var in reco:
            ...
            result[var] = ak.to_numpy(
                ak.concatenate((arrs[var][Pho0, 0], arrs[var][Pho1, 1]))
            )
        ...
        for var in hits_trimmed:
            ...
            result[bettername] = ak.concatenate(
                (arrs[Pho0Name][Pho0], arrs[Pho1Name][Pho1])
            )
    ...
    def readHGCAL(self):
        ...
        ].array()  # combined_rechit_y'].array()
        print("\ttook %0.3f seconds" % (time() - t0))

        t0 = time()
        print("Reading rechit_z...")
        Hit_Z_orig = self.tree[
            "comb_rechit_z_trimAhcal"
        ].array()  # combined_rechit_z'].array()
        print("\ttook %0.3f seconds" % (time() - t0))
        t0 = time()
        print("Reading rechit_z...")
        ...
        recHitEn = self.tree[
            "rechitEn_trimAhcal"
        ].array()  # combined_rechits_energy'].array()
        print("\ttook %0.3f seconds" % (time() - t0))
        ...
        frac = (
            ((Hit_Z < 54) * 0.0105)
            + (np.logical_and(Hit_Z > 54, Hit_Z < 154) * 0.0812)
            + ((Hit_Z > 154) * 0.12574)
        )
    ...
    def read(self, kind, N=None):
        ...
        arrs = arrs[[abs(j[0]) < 1.4442 and abs(j[1]) <1.4442 for j in arrs["Ele_Gen_Eta"]]]
        ...
        if hasgen[kind]:
            Ele1 = np.logical_and(
                reco_idxs == 0, ~noEle1
            )  # recoidx==0 and there is an ele1
        else:
        ...
        for var in gen + reco:  # per-particle information, flattened
            result[var] = ak.to_numpy(
                ak.concatenate(
                    (ak.flatten(arrs[var][Ele1]), ak.flatten(arrs[var][Ele2])
                )
            ))
        ...
        for var in hits:  # hit information, flattened
            ...
            #)
            result[name] = arrs[nameEle1][eventEle1]
        ...
        result["eventidx"] = np.concatenate(
            (eventEle1.nonzero()[0], eventEle2.nonzero()[0])
        )
        ...
        total = LeadElectronP4 + SubLeadElectronP4
        result["target"] = getMass(LeadElectronP4 + SubLeadElectronP4)
