...
class GravBlock(nn.Module):
    def __init__(self, input_dim = 10, dense_dim = 64, spatial_dim = 5, learned_dim = 22, out_dim = 48, n_neighbors = 40, aggrs = ['add','mean','max']):
...
def compressed_loss(pred, target, weight=None):
    ...
    logtruth  = torch.log(target)
    print("logtruth",logtruth)
    logpred = torch.empty_like(pred)
    logpred = torch.log(pred)
    print("logpred",logpred)
    loss = torch.sum((logpred-logtruth)**2/logtruth)/batch_size
    print("loss",loss)
    return loss
